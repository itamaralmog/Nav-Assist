    ...
    try:
        ...
        for _ in cell_tower_list:
            if sortedDis[0][0] == _["Site ID"]:
                r0 = ericson(float(_["freq"]),float(_["Height (m)"]),float(_["Power (dBm)"]),-80)
            if sortedDis[1][0] == _["Site ID"]:
                r1 = ericson(float(_["freq"]),float(_["Height (m)"]),float(_["Power (dBm)"]),-80)
            if sortedDis[2][0] == _["Site ID"]:
                r2 = ericson(float(_["freq"]),float(_["Height (m)"]),float(_["Power (dBm)"]),-80)
        print(r0,r1,r2)
